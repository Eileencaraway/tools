    ...
    def readFile3d(self, readfile, verbose=False):
        ...
        for line in open(readfile):
            ...
            if cline > 9:
                ...
                self.omegax[id] = float(tmp[13])
                self.omegay[id] = float(tmp[14])
                self.omegaz[id] = float(tmp[15])
                self.rad[id]    = float(tmp[16])
                #


                if len(tmp) > 17:
                    self.dirx[id]  = float(tmp[17])
                    self.diry[id]  = float(tmp[18])
                    self.dirz[id]  = float(tmp[19])
                    self.thtx[id]  = float(tmp[20])
                    self.thty[id]  = float(tmp[21])
                    self.thtz[id]  = float(tmp[22])
