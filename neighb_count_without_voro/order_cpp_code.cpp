...
int main(int argc, char **argv) {  //argc is the number of arguments, argc is a list of pointer to the arguments
     ...
     //sprintf(buffer,"cat %s|less|grep 'ITEM: NUMBER' -A 1|tail -n 1|awk '{print $1}' >> out1",filename);
     //int check = system(buffer);
     //FILE* in1=fopen("out1","r"); // FILE under C
     //check = fscanf(in1,"%d",&N); //define the box size
     //===============get the box size
     sprintf(buffer,"cat %s |less|grep 'ITEM: BOX' -A 1|tail -n 1|awk '{print $2}'>> out2",filename);
     int check = system(buffer);
     FILE* in2=fopen("out2","r"); // FILE under C
     check = fscanf(in2,"%lf",&L); //define the box size
     HL = L/2;  // give the value again
     //// //======read Trajectory
     sprintf(buffer,"cat %s |tail -n %d | awk '{print $1,$2,$3,$4,$14}' > cleanfile.txt", filename,N);
     check=system(buffer);
     ReadConf();
...
void outputdata(double &q6global){
  int i;
   ...
   for(int n = 0; n < N; n++){
	   ...
	   local_density<<n<<"     "<<4*PI*pow(Particles[n].Radius,3)/(3*Particles[n].Volume)<<endl;
	}
   GOP<<"Q6_global:"<<endl;
   GOP<<q6global<<endl;
   LOP.close();
