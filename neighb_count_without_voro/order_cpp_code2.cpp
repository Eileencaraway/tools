...
void outputdata(double &q6global, int &timestep, double &L);
//int argc, char** argv
int main(int argc, char **argv) {  //argc is the number of arguments, argc is a list of pointer to the arguments
	 char buffer[100];
	 int i,j,step,s_witch, count, m,timestep;
     double q6global;
     ...
     // cerr << filename << endl;
     //===============get the timestep
     sprintf(buffer,"cat %s |less|grep 'ITEM: TIMESTEP' -A 1|tail -n 1|awk '{print $1}'>>tout",filename);
     int check = system(buffer);
     FILE* tin=fopen("tout","r");
     check = fscanf(tin,"%d",&timestep); // get the timestep
     //================get the box size
     sprintf(buffer,"cat %s |less|grep 'ITEM: BOX' -A 1|tail -n 1|awk '{print $2}'>> out",filename);
     check = system(buffer);
     FILE* in=fopen("out","r"); // FILE under C
     check = fscanf(in,"%lf",&L); //define the box size
     HL = L/2;  // half of the boxsize

     //// //======read Trajectory
     sprintf(buffer,"cat %s |tail -n %d | awk '{print $1,$2,$3,$4,$17}' > cleanfile.txt", filename,N);
     check=system(buffer);
     ReadConf();
    ...
    pcon.import("cleanfile.txt"); // put the new file name
    ...
    //out put information
    outputdata(q6global,timestep,L);
	return 0;
}

// writ another code to clean the orginal code to produce the new code
...
void ReadConf(){
  ...
  // use ifstream
  // run over ifstream
  // read from this file to get the size of the box L , which is the 6 row and 2 number
  ConfFile.open("cleanfile.txt");
	for(int n = 0; n < N; n++){
	  ConfFile>>i>>Particles[n].R[0] >> Particles[n].R[1] >>Particles[n].R[2]>> Particles[n].Radius;
    }
	ConfFile.close();
 // save the clean data in another new file
...
void outputdata(double &q6global, int &timestep, double &L){
  int i;
   ...
   local_density<<"0.0 "<<L<<endl;
   local_density<<"0.0 "<<L<<endl;
   local_density<<"ITEM: ATOMS id x y z ld radius" <<endl;
   for(int n = 0; n < N; n++){
	   LOP<<n<<"     "<<Particles[n].q6<<endl;
       //
	   local_density<<n<<" "<<Particles[n].R[0]<<" "<<Particles[n].R[1]<<" "<<Particles[n].R[2]<<" "<<4*PI*pow(Particles[n].Radius,3)/(3*Particles[n].Volume)<<" "<<Particles[n].Radius<<endl;
	}
   GOP<<"Q6_global:"<<endl;
   GOP<<q6global<<endl;
   LOP.close();
