...
int main() {
    ...
    // char* filename =  argv[1];
     //===============
     //sprintf(buffer,"cat dumpfile.0100000000.txt |less|grep 'ITEM: BOX' -A 1|tail -n 1|awk '{print $2}'>> out");
     //system(buffer);
     // FILE* in=fopen("out","r");
     // fscanf(in,"%lf",&L); //define the box size
    //// //======read Trajectory
    ReadConf();
...
void outputdata(double &q6global){
  int i;
   ...
   for(int n = 0; n < N; n++){
	   ...
	   local_density<<n<<"     "<<1.333*PI*pow(Particles[n].Radius,3)/Particles[n].Volume<<endl;
	}
   GOP<<"Q6_global:"<<endl;
   GOP<<q6global<<endl;
   LOP.close();
