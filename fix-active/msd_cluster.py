...
# calculate the msd of the particle in cluster only. 
import sys
import os
import glob
import StringIO
...
def onefile(tsrt,tend,clusterid):
    ...
    for i in range(nid):
        id = clusterid[i] -1  ## id need below minus one in the class
        dxu[i]=lstr.xu[id]-lend.xu[id]
        dyu[i]=lstr.yu[id]-lend.yu[id]
        if dim==3:
            dzu[i]=lstr.zu[id]-lend.zu[id]
...
for k in range(1,len(filename)):
    ...
    #print dend
    dend = filename[k]
    msd[k],t[k]=onefile(dsrt,dend,clusterid)
    t[k]=t[k]*dt
