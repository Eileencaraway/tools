...
class LammpsOperations:
    ...
    def initialize(self):
        ...
        self.iz  = [0 for i in range(self.nb_atoms)]
    ...
    def readFile3d(self, readfile, verbose=False):
        ...
        for line in open(readfile):
            ...
            if cline > 9:
                ...
                self.rad[id]    = float(tmp[7])
                self.ADx[id]    = float(tmp[8])
                self.ADy[id]    = float(tmp[9])
                self.ADz[id]    = float(tmp[10])
                ...
                if len(tmp) > 11:
                    self.dirx[id]= float(tmp[11])
                    self.diry[id]= float(tmp[12])
                    self.dirz[id]= float(tmp[13])
                    self.vx[id]  = float(tmp[14])
                    self.vy[id]  = float(tmp[15])
